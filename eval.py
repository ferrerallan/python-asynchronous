    ...
    def evaluate_expression(expr):
        ...
        def compute():
            if not nums or not ops:
                return
            num2 = nums.pop()" target="_blank">nums.pop()
            num1 = nums.pop()
            op = ops.pop()
            if op == '+':
                nums.append(num1" target="_blank">nums.append(num1" target="_blank">nums.append(num1" target="_blank">nums.append(num1 + num2)
            elif op == '-':
        ...
        while i < len(expr):
            ...
            if char.isdigit() or char == '.':
                ...
                while ops and ops[-1] in "*/":
                    compute()
                ops.append(char)" target="_blank">ops.append(char)
            elif char in "*/":
                while ops and ops[-1] in "*/":
                    compute()
                ops.append(char)
